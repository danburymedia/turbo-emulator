...
pub struct Riscv64DebugWrapper {
    pub icpu: RiscvInt,
    pub breakpoints: Vec<u64>,
    pub exec_mode: DebugExecMode,

}
impl Riscv64DebugWrapper {
...
impl run_blocking::BlockingEventLoop for EmuGdbEventLoop {
    type Target = Riscv64DebugWrapper;
...
impl Target for Riscv64DebugWrapper {
...
impl SingleThreadBase for Riscv64DebugWrapper {
...
impl SingleRegisterAccess<()> for Riscv64DebugWrapper {
    fn read_register(&mut self,
                     tid: (),
                     reg_id: gdbstub_arch::riscv::reg::id::RiscvRegId<u64>,
                     buf: &mut [u8]) -> TargetResult<usize, Self> {
        match reg_id {
            RiscvRegId::Gpr(g) => {
                let val = self.icpu.regs[g as usize] as u64;
                buf.copy_from_slice(&val.to_le_bytes());
                Ok(buf.len())
            }
            RiscvRegId::Fpr(f) => {
                let val = self.icpu.fregs[f as usize] as u64;
                buf.copy_from_slice(&val.to_le_bytes());
                Ok(buf.len())
            }
            RiscvRegId::Pc => {
                let val = self.icpu.get_pc_of_current_instr() as u64;
                buf.copy_from_slice(&val.to_le_bytes());
                Ok(buf.len())
            }
            RiscvRegId::Csr(c) => {
                let val = self.icpu.get_csr_raw(c as usize) as u64;
                buf.copy_from_slice(&val.to_le_bytes());
                Ok(buf.len())
            }
            RiscvRegId::Priv => {
                let val = get_privilege_encoding(self.icpu.prvmode) as u64;
                buf.copy_from_slice(&val.to_le_bytes());
                Ok(buf.len())
            }
...
impl Breakpoints for Riscv64DebugWrapper {
...
impl SwBreakpoint for Riscv64DebugWrapper {
...
impl SingleThreadResume for Riscv64DebugWrapper {
...
impl SingleThreadSingleStep for Riscv64DebugWrapper {
